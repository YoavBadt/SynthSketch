<template>
    <div class="filter">
            ...
            <div class="viz_head">
                <div class="filter_mode_select">
                    <template v-for="(n,i) in filterModes">
                        <input type="radio" :id="n+name" :name="`filter-${name}`" :value="n" :checked="filter.mode === n"  @change="updateFilter(n,'mode')"/> 
        ...
        <div class="filter_knobs">
            <SmallKnob name="cutoff" @update:value="updateFilter" :value="filter.cutoff" :min="filter.cutMin" :max="filter.cutMax" :step="1"/>
            <SmallKnob name="mix" @update:value="updateFilter" :value="filter.mix" :min="filter.mixMin" :max="filter.mixMax" :step="1"/>
            <SmallKnob name="resonance" @update:value="updateFilter" :value="filter.resonance" :min="filter.resMin" :max="filter.resMax" :step="1"/>
            <!-- <CutSlider /> -->
            <!-- <ResoSlider/> -->
             <!-- <Knob show="show" name="cut" :style="{marginTop : '-32px'}"/> -->
             <!-- <Knob show="show" name="mix" :style="{marginTop : '-32px'}"/> -->
             <!-- <Knob show="show" name="rez" :style="{marginTop : '-32px'}"/> -->
        </div>
...
export default {
    ...
    methods:{
        updateFilter(payload,name){
            store.updateFilter(this.name,payload,name)
        }
    }
}
</script>
